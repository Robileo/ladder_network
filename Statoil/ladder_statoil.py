...
layers = [{"type": "conv2d", "kernel_shape": [3,3,2,16]}, # 28x28x1 -> 28x28x32
           ...
           {"type": "max_pool_2x2"},                      # 28x28x32 -> 14x14x32
           {"type": "conv2d", "kernel_shape": [3,3,16,32]},
           {"type": "conv2d", "kernel_shape": [3,3,32,32]},# 14x14x32 -> 14x14x64
           {"type": "max_pool_2x2"},                       # 14x14x64 -> 7x7x64
           {"type": "conv2d", "kernel_shape": [3,3,32,64]},
           {"type": "conv2d", "kernel_shape": [3,3,64,64]},# 14x14x32 -> 14x14x64
           {"type": "max_pool_2x2"},
           {"type": "conv2d", "kernel_shape": [3,3,64,128]},
           {"type": "conv2d", "kernel_shape": [3,3,128,128]},
           {"type": "max_pool_2x2"},
           {"type": "conv2d", "kernel_shape": [3,3,128,256]},
           {"type": "conv2d", "kernel_shape": [3,3,256,256]},
           {"type": "max_pool_2x2"},
           {"type": "flat"},                               # 7x7x64 -> 3136
           {"type": "dense", "kernel_shape": [3*3*256, 1700]},# 3136 -> 1024
           {"type": "dense", "kernel_shape": [1700, 1024]},
...
decay_after = 5  # epoch after which to begin learning rate decay
batch_size = 100
...
initial_accuracy = sess.run(accuracy, feed_dict={
                                      feedforward_inputs: data_sat.validation.images, 
                                      outputs: data_sat.validation.labels, 
                                      training: False})
...
for i in range(i_iter, num_iter):
    labeled_images, labels, unlabeled_images = data_sat.train.next_batch(batch_size)
...
#    sys.stdout.flush()
    ...
    if (i > 1) and ((i+1) % (num_iter/num_epochs) == 0):
        ...
        train_acc  = sess.run(accuracy, feed_dict={feedforward_inputs: data_sat.train.labeled_ds.images, 
                                                   outputs: data_sat.train.labeled_ds.labels, 
                                                   training: False})
    
        test_acc = sess.run(accuracy, feed_dict={feedforward_inputs: data_sat.validation.images, 
                                                 outputs: data_sat.validation.labels, 
                                                 training: False})
...
final_accuracy = sess.run(accuracy, feed_dict={feedforward_inputs: data_sat.validation.images, 
                                          outputs: data_sat.validation.labels, 
                                          training: False})
